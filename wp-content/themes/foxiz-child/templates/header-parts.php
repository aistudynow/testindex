...
if ( ! function_exists( 'foxiz_render_text_logo' ) ) {
	...
	<div class="<?php echo strip_tags( $class_name ); ?>">
		<?php if ( is_front_page() && ! isset( $GLOBALS['foxiz_h1_rendered'] ) ) :
...
if ( ! function_exists('foxiz_get_logo_picture') ) {
	function foxiz_get_logo_picture( $default, $dark = [], $classes = 'logo-picture', $loading = 'eager' ) {
		if ( empty( $default['url'] ) ) {
			return '';
		}

		$w = ! empty( $default['width'] )  ? (int) $default['width']  : 0;
		$h = ! empty( $default['height'] ) ? (int) $default['height'] : 0;

		$is_lazy = ( $loading !== 'eager' );

		// <img> gets the priority hints (not <source>)
		$img_attr  = ' alt="' . esc_attr( get_bloginfo('name') ) . '" decoding="async"';
		$img_attr .= $w ? ' width="' . esc_attr($w) . '"' : '';
		$img_attr .= $h ? ' height="' . esc_attr($h) . '"' : '';
		$img_attr .= $is_lazy ? ' loading="lazy" fetchpriority="low"' : ' loading="eager"';

		$out  = '<picture class="' . esc_attr($classes) . '">';

		// optional dark logo via prefers-color-scheme
		if ( ! empty( $dark['url'] ) ) {
			$out .= '<source media="(prefers-color-scheme: dark)" srcset="' . esc_url( $dark['url'] ) . '">';
		}

		// default (light) logo as the <img> fallback
		$out .= '<img src="' . esc_url( $default['url'] ) . '"' . $img_attr . '>';
...
// One <picture> for light/dark; default is eager (no fetchpriority=high)
echo foxiz_get_logo_picture( $settings['logo'], $settings['dark_logo'], 'logo-picture desktop', 'eager' );

// Optional transparent variant (lazy/low), shown only in a transparent header state via CSS
if ( ! empty( $settings['transparent_logo']['url'] ) ) {
    echo foxiz_get_logo_picture( $settings['transparent_logo'], [], 'logo-picture logo-transparent desktop', 'lazy' );
}
	...
	function foxiz_render_mobile_logo( $settings = [] ) {
		...
		<div class="<?php echo implode( ' ', $class_name ); ?>">
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo strip_tags( $blog_name ) ?>">
				...
				if ( empty( $settings['dark_mobile_logo']['url'] ) ) {
    $settings['dark_mobile_logo'] = ! empty( $settings['dark_logo']['url'] )
        ? $settings['dark_logo']
        : $settings['mobile_logo'];
}

// mobile version; default eager (no fetchpriority=high)
echo foxiz_get_logo_picture( $settings['mobile_logo'], $settings['dark_mobile_logo'], 'logo-picture mobile', 'eager' );
    ...
    function foxiz_header_user( $settings = [] ) {
        ...
        <div class="wnav-holder widget-h-login header-dropdown-outer">
            <?php if ( is_user_logged_in() && ! is_admin() ) :
                ...
                $wd4_full_nav_name      = '';
                $wd4_compact_nav_name   = '';
                $wd4_header_first_name  = '';
                $wd4_header_last_name   = '';
                $wd4_header_short_name  = '';
                $wd4_profile_flash      = [];
                $wd4_profile_redirect   = function_exists( 'wd4_get_current_url' ) ? wd4_get_current_url() : foxiz_get_current_permalink();

                if ( $current_user instanceof WP_User ) {
                    $wd4_full_nav_name    = foxiz_strip_tags( $current_user->display_name );
                    $wd4_compact_nav_name = $wd4_full_nav_name;

                    $wd4_header_first_name = get_user_meta( $current_user->ID, 'first_name', true );
                    $wd4_header_last_name  = get_user_meta( $current_user->ID, 'last_name', true );
                    $wd4_header_short_name = get_user_meta( $current_user->ID, 'wd4_short_display_name', true );

                    if ( function_exists( 'wd4_consume_profile_flash' ) ) {
                        $wd4_profile_flash = wd4_consume_profile_flash( (int) $current_user->ID );
                    }

                    if ( function_exists( 'wd4_get_compact_user_name' ) ) {
                        $resolved_compact = wd4_get_compact_user_name( $current_user );
                        if ( '' !== $resolved_compact ) {
                            $wd4_compact_nav_name = foxiz_strip_tags( $resolved_compact );
                        }
                    }
                }
                ...
                <div class="logged-user-control">
                    <a class="logged-profile-link header-element" href="<?php echo esc_url( $profile_url ); ?>" rel="nofollow" aria-label="<?php foxiz_html_e( 'View your profile', 'foxiz' ); ?>">
                        ...
                        <?php endif; ?>
                        <span class="logged-welcome screen-reader-text" title="<?php echo esc_attr( $wd4_full_nav_name ); ?>">
                            <?php echo foxiz_html__( 'Hi,', 'foxiz' ) . '<strong>' . esc_html( $wd4_compact_nav_name ) . '</strong>'; ?>
                        </span>
                    </a>
                </div>

                <!-- Dropdown content: Edit profile + menu + logout -->
                <div class="header-dropdown user-dropdown">
                    <?php if ( $current_user instanceof WP_User ) : ?>
                        <button
                            type="button"
                            class="wd4-header-profile__edit-link"
                            data-wd4-profile-modal-open
                        >
                            <?php esc_html_e( 'Edit profile', 'foxiz-child' ); ?>
                        </button>
                    <?php endif; ?>
                    ...
                    <div class="logout-wrap">
                        <a class="logout-url" href="<?php echo wp_logout_url( $logout_redirect ); ?>" rel="nofollow">
                            <?php echo foxiz_html__( 'Sign Out', 'foxiz' ); ?>
                            <i class="rbi rbi-logout"></i>
                        </a>
                    </div>
                </div>
                ...
                <?php if ( $current_user instanceof WP_User ) : ?>
                    <!-- Profile modal -->
                    <div class="wd4-profile-modal" id="wd4-profile-modal" hidden>
                        <div class="wd4-profile-modal__backdrop" data-wd4-profile-modal-close></div>

                        <div class="wd4-profile-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="wd4-profile-modal-title">
                            <div class="wd4-profile-modal__header">
                                <h2 class="wd4-profile-modal__title" id="wd4-profile-modal-title">
                                    <?php esc_html_e( 'Edit profile', 'foxiz-child' ); ?>
                                </h2>
                                <button
                                    type="button"
                                    class="wd4-profile-modal__close"
                                    data-wd4-profile-modal-close
                                    aria-label="<?php esc_attr_e( 'Close', 'foxiz-child' ); ?>"
                                >
                                    &times;
                                </button>
                            </div>

                            <?php if ( ! empty( $wd4_profile_flash['message'] ) ) :
                                $notice_classes = 'wd4-header-profile__notice';
                                if ( isset( $wd4_profile_flash['type'] ) && 'error' === $wd4_profile_flash['type'] ) {
                                    $notice_classes .= ' wd4-header-profile__notice--error';
                                } else {
                                    $notice_classes .= ' wd4-header-profile__notice--success';
                                }
                                ?>
                                <p class="<?php echo esc_attr( $notice_classes ); ?>">
                                    <?php echo esc_html( $wd4_profile_flash['message'] ); ?>
                                </p>
                            <?php endif; ?>

                            <div class="wd4-profile-modal__body">
                                <form class="wd4-header-profile__form" method="post">
                                    <label class="wd4-header-profile__label" for="wd4_header_first_name">
                                        <?php esc_html_e( 'First name', 'foxiz-child' ); ?>
                                        <input
                                            type="text"
                                            id="wd4_header_first_name"
                                            name="wd4_profile_first_name"
                                            value="<?php echo esc_attr( $wd4_header_first_name ); ?>"
                                            maxlength="40"
                                            autocomplete="given-name"
                                        >
                                    </label>

                                    <label class="wd4-header-profile__label" for="wd4_header_last_name">
                                        <?php esc_html_e( 'Last name', 'foxiz-child' ); ?>
                                        <input
                                            type="text"
                                            id="wd4_header_last_name"
                                            name="wd4_profile_last_name"
                                            value="<?php echo esc_attr( $wd4_header_last_name ); ?>"
                                            maxlength="40"
                                            autocomplete="family-name"
                                        >
                                    </label>

                                    <label class="wd4-header-profile__label" for="wd4_header_short_name">
                                        <?php esc_html_e( 'Nickname (optional)', 'foxiz-child' ); ?>
                                        <input
                                            type="text"
                                            id="wd4_header_short_name"
                                            name="wd4_profile_short_name"
                                            value="<?php echo esc_attr( $wd4_header_short_name ); ?>"
                                            maxlength="40"
                                            autocomplete="nickname"
                                        >
                                    </label>

                                    <?php wp_nonce_field( 'wd4_update_profile_fields', 'wd4_profile_fields_nonce' ); ?>
                                    <input type="hidden" name="wd4_profile_redirect" value="<?php echo esc_url( $wd4_profile_redirect ); ?>">

                                    <button type="submit" class="wd4-header-profile__submit">
                                        <?php esc_html_e( 'Save profile', 'foxiz-child' ); ?>
                                    </button>
                                </form>

                                <form class="wd4-header-avatar__form" method="post" enctype="multipart/form-data">
                                    <label class="wd4-header-profile__label" for="wd4_header_avatar">
                                        <?php esc_html_e( 'Profile image', 'foxiz-child' ); ?>
                                        <input
                                            type="file"
                                            id="wd4_header_avatar"
                                            name="wd4_author_avatar"
                                            accept="image/*"
                                        >
                                    </label>

                                    <?php wp_nonce_field( 'wd4_update_author_avatar', 'wd4_author_avatar_nonce' ); ?>
                                    <input type="hidden" name="wd4_avatar_redirect" value="<?php echo esc_url( $wd4_profile_redirect ); ?>">

                                    <button type="submit" class="wd4-header-profile__submit">
                                        <?php esc_html_e( 'Save image', 'foxiz-child' ); ?>
                                    </button>
                                </form>
                            </div>
                        </div>
            ...
            <?php else : // not logged in ?>
                <?php if ( empty( $settings['header_login_layout'] ) ) : ?>
                    <a href="<?php echo wp_login_url( $login_redirect ); ?>" class="login-toggle is-login header-element" data-title="<?php echo esc_attr( $login_label ); ?>"<?php if ( ! foxiz_get_option( 'disable_login_popup' ) ) {
                        echo ' role="button"';
                ...
                <?php elseif ( '1' === $settings['header_login_layout'] ) : ?>
                    <a href="<?php echo wp_login_url( $login_redirect ); ?>" class="login-toggle is-login is-btn header-element" rel="nofollow"<?php if ( ! foxiz_get_option( 'disable_login_popup' ) ) {
                        echo ' role="button"';
                    } ?> aria-label="<?php echo esc_attr( $login_label ); ?>">
                        <span><?php foxiz_render_inline_html( $login_label ); ?></span>
                    </a>
                <?php else : ?>
                    <a href="<?php echo wp_login_url( $login_redirect ); ?>" class="login-toggle is-login is-btn is-btn-icon header-element" rel="nofollow"<?php if ( ! foxiz_get_option( 'disable_login_popup' ) ) {
                        echo ' role="button';
                    } ?> aria-label="<?php echo esc_attr( $login_label ); ?>">
                        <?php
                        if ( ! empty( $settings['login_icon'] ) ) {
...
if ( ! function_exists( 'foxiz_header_search' ) ) {
	function foxiz_header_search( $settings = [] ) {
		...
		<div class="wnav-holder w-header-search header-dropdown-outer">
			<a href="#" role="button" data-title="<?php foxiz_html_e( 'Search', 'foxiz' ); ?>" class="<?php echo strip_tags( join( ' ', $classes ) ); ?>" aria-label="<?php esc_attr_e( 'Search', 'foxiz' ); ?>">
		...
		<div class="mobile-collapse">
			<div class="collapse-holder">
				<div class="collapse-inner">
					...
					<?php if ( ! empty( $settings['collapse_template'] ) && ( empty( $settings['collapse_template_display'] ) || 'replace' !== $settings['collapse_template_display'] ) && ! foxiz_is_amp() ) {
								...
								<?php if ( ! is_user_logged_in() ) : ?>
									...
									<a href="<?php echo wp_login_url( $login_redirect ); ?>" class="login-toggle is-login is-btn" rel="nofollow"><?php foxiz_html_e( 'Sign In', 'foxiz' ); ?></a>
                                                                <?php else :
                                                                        ...
                                                                        $wd4_mobile_full_name    = '';
                                                                        $wd4_mobile_compact_name = '';

                                                                        if ( $current_user instanceof WP_User ) {
                                                                                $wd4_mobile_full_name    = foxiz_strip_tags( $current_user->display_name );
                                                                                $wd4_mobile_compact_name = $wd4_mobile_full_name;

                                                                                if ( function_exists( 'wd4_get_compact_user_name' ) ) {
                                                                                        $resolved_mobile_compact = wd4_get_compact_user_name( $current_user );
                                                                                        if ( '' !== $resolved_mobile_compact ) {
                                                                                                $wd4_mobile_compact_name = foxiz_strip_tags( $resolved_mobile_compact );
                                                                                        }
                                                                                }
                                                                        }
                                                                        $logout_redirect = foxiz_get_option( 'logout_redirect' );
                                                                        ...
                                                                        ?>
                                                                        <span class="mobile-login-title" title="<?php echo esc_attr( $wd4_mobile_full_name ); ?>"><?php echo foxiz_html__( 'Hi,', 'foxiz' ) . '<strong>' . esc_html( $wd4_mobile_compact_name ) . '</strong>'; ?></span>
...
if ( ! function_exists( 'foxiz_header_mini_cart_html' ) ) {
	function foxiz_header_mini_cart_html( $dropdown_section = true ) {
		...
		<aside class="header-mini-cart wnav-holder header-dropdown-outer">
			<a class="<?php echo strip_tags( $class_name ); ?>" href="<?php echo esc_url( wc_get_cart_url() ) ?>" data-title="<?php foxiz_attr_e( 'View Cart', 'foxiz' ); ?>" aria-label="<?php foxiz_attr_e( 'View Cart', 'foxiz' ); ?>">
